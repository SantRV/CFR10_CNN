    ...
    def visualise_data(self, num_images: int = 25):
        ...
        for i in range(num_images):
            ...
            for j in range(len(self.train_labels[i])):
                if self.train_labels[i][j] == 1:
                    classLabel = j
                    break

            plt.xlabel(self.class_names[classLabel])

            plt.show()

    def visualise_classes(self, num_images: int = 25):
        # Visualizing some of the images from the training dataset
        plt.figure(figsize=[10, 10])

        data = np.array(self.train_labels)
        # Count the occurrences of each class label
        class_counts = np.sum(data, axis=0)

        # class_counts = np.zeros((1, 10))[0]
        ...
        print("DATA FRAME", len(data), class_counts)
